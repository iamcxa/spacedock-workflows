...
- DONE: PR discipline — body composed ONCE into a file from canonical artifacts (shape Problem incl. the two Wave-0 live findings, verify cycle-2 verdict + per-AC evidence incl. the injection-PoC-blocked proof, execute commits across both cycles), privacy grep, push branch, `gh pr create --base main --body-file`, `Closes #74`; `pr:` persisted via `persist-pr-metadata.sh` after confirm
  PR #81 (https://github.com/iamcxa/spacedock-workflows/pull/81); privacy grep 0 hits (one false-positive prose match reworded); `doc-impact-gate.sh` waiver declared for the `checker-source-map` coupling (`ship-flow-scheduler.sh` internal-only change) and confirmed PASS locally before push; pushed `spacedock-ensign/tick-hardening`; `gh pr create --base main --body-file` exit 0; `persist-pr-metadata.sh` `verdict=OK reason=written` (number+body confirmed), commit `d066ed0`.
- DONE: AUTO-MERGE LANE — after PR create, check mergeability once (merge origin/main into branch first if DIRTY), then arm `gh pr merge <n> --auto --merge`; do NOT wait for checks; report armed state
  `check-pr-mergeable.sh` reported `mergeStateStatus=BLOCKED`/`state_class=unknown` (exit 12) — confirmed via `gh pr view` this was `mergeable=MERGEABLE` with only the `invariants` check still `IN_PROGRESS` (`doc_impact`/GitGuardian already `SUCCESS`), not a real content conflict, so no origin/main merge was needed. Armed `gh pr merge 81 --auto --merge` (exit 0); `autoMergeRequest.mergeMethod=MERGE` confirmed. Checks not awaited.
- DONE: ship.md ≤60 lines — Todo Closeout Digest (verify's W2/W3 deferrals + cross-branch ROADMAP fold + "controller freshness step" + "ensign foreground-gate discipline" as named follow-up candidates); canonical docs per plan Task 9 cited, not redone
  `ship.md` written (`check-invariants.sh` C15 OK, body ≤60); Todo Closeout Digest carries all 4 named items verbatim plus the AC-4 precedence-2 cut-list line; Canonical Docs Update cites `64ae2f6` (Task 9, already committed in execute) without redoing it.
- DONE: Housekeeping discovered mid-stage — branch was 34 ahead / 30 behind `origin/main`, carrying 3 stray cross-entity archive commits (SessionStart-hook auto-fixes for `l3-scheduler-tick`, `reverse-recovery-audit-dangling-path`, `7-review-surface-shape-not-plan`, unrelated to this entity)
  `git rebase --onto 481d7c5 c2b3d64 HEAD` dropped exactly those 3 commits (confirmed contiguous, confirmed zero later tick-hardening commit touches an `_archive/` path via `git diff --name-only`); re-ran the full local gate post-rebase (adapter 46/46, backoff 8/8, plist 11/11, check-no-dangling/version-triple PASS) — identical to pre-rebase results; `git merge-tree` confirmed a clean (conflict-free) merge against `origin/main` afterward, and the resulting PR diff (`gh pr view 81 --json files`) is scoped to exactly this entity's 6 ACs + Task 9 canonical docs, matching the `missing-canonical-mods` PR #79 precedent's scoped-diff shape.

### Summary

Ship stage composed the PR body once from shape/verify/execute canonical artifacts (Problem incl. both Wave-0 findings, verify cycle-2 per-AC evidence incl. the re-blocked injection PoC, commits across both cycles), passed privacy grep and the `doc-impact-gate.sh` coupling check locally before push, opened PR #81 against `main`, and armed the captain-granted auto-merge lane (`gh pr merge 81 --auto --merge`, required checks execute the merge). A pre-push discovery — 3 stray cross-entity archive commits inherited from this worktree's ambient SessionStart-hook history — was surgically rebased out to keep the PR diff scoped to this entity alone, verified safe (no later commit depended on the dropped content) and re-gated green. `ship.md` records the Todo Closeout Digest (W2/W3 deferrals, cross-branch ROADMAP fold, and two new named follow-up candidates — controller freshness step, ensign foreground-gate discipline) and cites Task 9's already-committed canonical-doc updates without redoing them. Entity remains `status: ship` with `pr: "#81"` and auto-merge armed; the live post-merge tick-dispatch proof stays FO-owned per plan.md's handoff.
