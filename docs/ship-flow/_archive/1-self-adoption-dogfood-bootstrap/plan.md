...
<details>
<summary>Plan Imported Design DCs (generated import — non-UI entity, tables empty by design; see note)</summary>
...
Non-UI entity (`affects_ui: false`, `domain: schema`) — design.md's `### Hand-off
to Plan` carries prose-referenced constraints ("the seven Constraints for Plan
Stage bullets"), not the UI-typed `design_constraints[]` / `visible_surface_map[]`
/ `render_fidelity_targets[]` schema this importer parses, so the tables below
are correctly empty (0 imported = 0 source, no mismatch). The real constraints
are threaded through `## Plan Output → ### Research Summary`, `### D4 Decision`,
`### Verification Spec`, and per-task `desc`/`regression_risk` fields instead.
