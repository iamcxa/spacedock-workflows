...
| l3-scheduler-tick | Adds `plugins/ship-flow/bin/ship-flow-scheduler.sh` (`tick`/`report`/`rollup` subcommands), `lib/scheduler-lease.sh`, `lib/scheduler-runner-adapter.sh` — a deterministic, idempotent, stateless scheduler-tick atom: exactly one bounded action per invocation (dispatch \| advance \| reconcile \| no-op), structured JSON event out, exit. D1 (carrier-swap contract): the tick's ONLY carrier-specific seam is the runner-adapter interface (`entity, workdir, timeout, env` in; `exit_class, sentinel, receipt` JSON out) — launchd (now) and crewdock (later) are interchangeable carriers invoking the same atom; the tick never owns transcripts, park/resume, scavenging, or container lifecycle. D2: the daemon owns no state of record — canonical entity frontmatter + `gh` reads are truth; the gate-projection report and controller lease are re-derived every invocation, never a writable ledger. D3: concurrency=1 via a controller-worktree mkdir-atomic lease with liveness-only stale reclaim (a still-alive holder is never stolen from, however old its record). D4 (reverse-recovery): composes existing WORKING primitives (`dag-waves.sh --ready`, `merged-pr-closeout-reconciler.sh`, `spacedock status`) unmodified — rebuilds none. |
...
| 006.1-attempt-circuit-protocol-and-clock-authority | Defines a plan/execute-only `stage-attempt-v1` contract that binds FO-issued attempt identity to the active lease and Git/completion evidence, keeps `completion-v1` bytes frozen, and permits nonterminal resume only under same-boot, non-regressing monotonic authority while failing closed on clock loss or regression. |
| 006-plan-attempt-vertical | Routes only the real First Officer `plan` caller through one fresh bounded `stage-attempt-v1`: caller-owned begin/WAL authority wraps the worker’s unchanged `completion-v1` receipt, completion checkpointing remains separate from Contract 1, and one passed terminal history/return contribution closes the attempt. Non-plan stages retain completion-only handling; recovery and execute generalization remain deferred. |
<!-- /section:decisions -->
