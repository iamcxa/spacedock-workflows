...
## Post-merge closeout trigger convergence

Both merge paths converge after authoritative provider evidence reports the
implementation PR as merged. `bin/merged-pr-closeout-reconciler.sh` owns the
terminal projection, landing proof, debrief, final ship record, archive,
ROADMAP transition, and idempotency validation as one receipt-bound Git
...
The closeout receipt remains the sole idempotency sentinel. New trigger paths
must reuse it through the reconciler rather than introduce parallel markers or
deferred terminal states.
