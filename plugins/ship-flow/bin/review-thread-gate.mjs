...
const REVIEW_THREAD_ADJUDICATION_MESSAGE =
  "Unresolved review thread targets the current PR head; route to SO/EM adjudication to answer in-thread with fixed, push-back: false positive, or needs captain decision, include code/test/SO-EM judgment evidence via gh api, then re-trigger the AI reviewer gate. Do not rely on author self-approval.";
...
function unresolvedThreadIssue({ payloadPath, thread, expectedHead }) {
  ...
  return issue(
    ...
    payloadPath,
    "Unresolved review thread is not outdated but has no comment bound to the current head; route to SO/EM adjudication to answer in-thread with fixed, push-back: false positive, or needs captain decision, include code/test/SO-EM judgment evidence via gh api, then re-trigger the AI reviewer gate. Do not rely on author self-approval.",
