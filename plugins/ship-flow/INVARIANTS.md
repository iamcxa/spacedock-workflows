...
**Rule**: Any commit that mutates an entity's `status:` YAML frontmatter field on `docs/<workflow>/<id>-<slug>/index.md` (folder layout) or `docs/<workflow>/<id>-<slug>.md` (flat) MUST carry one of three sanctioned transition receipts. Stage entry and feedback routing are owned by the First Officer; stage completion is owned by `plugins/ship-flow/lib/advance-stage.sh` (or its sub-helpers invoked by the same call chain). Direct YAML edit + arbitrary manual `git commit` is forbidden.
...
**Feedback-stage receipt (FO)**: the commit **subject** must match `feedback: <non-empty summary> <rejected-stage> cycle <positive-integer> to <target-stage>`. For every entity mutated by the commit, C14 binds the parent status to `<rejected-stage>`, the resulting status to `<target-stage>`, and requires exactly one matching record in one `### Feedback Cycles` section. That record must carry the same cycle, `rejected_stage`, and `feedback_to`, plus `captain_decision: fix`, a calendar-valid UTC `routed_at` timestamp, and `verify_artifact: <rejected-stage>.md@<7-40 lowercase hex>`. The subject is authoritative; canonical-looking text in the commit body cannot manufacture this receipt.
...
**Grep check** (Tier A automated): `check_entity_status_via_advance_stage_only()` in `plugins/ship-flow/bin/check-invariants.sh` (legacy check name retained for compatibility). Scans commits ahead of `git merge-base origin/main HEAD` for entity-index modifications; compares frontmatter `status:` before/after each changed entity file; accepts a canonical FO stage-entry subject only when its named stage matches every after-state, or a canonical FO feedback subject only when every mutated path has matching before/after stages and Feedback Cycles evidence, otherwise requires the existing `: advance status to ` completion receipt. Skips gracefully when `origin/main` ref is absent (e.g., fresh test fixtures).

**v1 limitation — completion body-forgery false negative**: the `advance-stage.sh` substring match still runs against the full commit message INCLUDING body. A committer aware of that completion signature can include `": advance status to "` in the body to bypass detection. FO stage-entry and feedback-stage receipts do not share this limitation because they are subject-only and state-bound; feedback receipts additionally bind body evidence on every mutated entity path. v1 remains calibrated for **accidental** bypass; v2 hardening (`docs/ship-flow/todos/enforce-advance-stage-primitive-only-v2-subject-only-match.md`) tightens completion receipts to subject-line-only OR requires a commit trailer like `Stage-Advance-Tool: advance-stage.sh@<sha>`.
