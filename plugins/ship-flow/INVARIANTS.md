...
- **Utility-skills (uncapped)**: `add-todos`, `ship-onboard`, `ship-runtime-detect`, `domain-registry`, `ui-verify`, `test-driven-development`, `verify-reviewer-panel`, `doc-sync`, `distill-reference`, `codex-gate`, `harvest-decide`, `memory-cleanup`, `ship-epic`, `ship-project`, `science-officer-em` — 15 total. Utility scripts/mods such as `ship-flow-lint` and `debrief-guardrail-harvest` are non-stage, non-skill guardrails and do not count against the stage-skill cap. (`ship-epic` is a cross-entity epic→children wave orchestrator — it sequences invocations of the existing single-entity `/ship` pipeline by `depends-on`; it does NOT re-implement any stage, so it is a utility skill, not a stage-orchestration re-implementation per Principle 2 trip condition 3.) (`ship-project` is the project-intake entry point — it sequences the deterministic intake libs `issues-to-contract.sh` → `instantiate-cut-project.sh` around one Linear MCP fetch and STOPS at entities+plan; it does NOT run any stage, so it is a utility skill.) (`science-officer-em` is an on-demand judgment/report entry point over the standing EM profile; it does NOT advance stages or own dispatch, so it is utility, not stage orchestration.)
...
### Principle 15: Two Independent Entity Transition Contracts

**Rule**: A commit that changes an existing entity's frontmatter `status:` has exactly two owner-bearing contracts. Both are checked against the owning workflow's `stages.states` graph as it existed at the commit's parent. A transition MUST be either the declared next stage or the source stage's declared `feedback-to` target before a receipt can authorize it. Direct YAML edit plus an arbitrary manual commit is forbidden.

**Contract 1 — First Officer stage entry**: the First Officer owns entry into a stage. Its commit subject, and only its subject, MUST match `dispatch: <nonempty summary> entering <stage>` for a fresh dispatch or `advance: <nonempty summary> entering <stage>` for same-worker continuation. The named stage MUST equal every mutated entity's after-status. Body-only, empty-summary, lookalike, wrong-stage, skipped, backward, and mixed legal/illegal transitions are rejected. This contract enters the stage; it does not register worker completion.

**Contract 2 — stage-worker completion**: a stage worker owns registration of the already-current stage on canonical folder entities. It invokes `advance-stage.sh` with one of the six exhaustive status/stage/file triples. The helper verifies identical HEAD/index/worktree entity and artifact snapshots, unchanged current status, canonical matching registries, and caller hash; constructs a detached one-path completion commit; verifies it; and publishes only the attached branch ref with one compare-and-swap. It never performs Contract 1, changes status, writes the live index/worktree, rolls a ref back, repairs stale registries, or migrates a legacy entity. `--commit-as` is audit text, not transition authority. Exactly one verified `published` or `already-registered` receipt is emitted on success and no receipt on failure.

**FO same-run checkpoint**: after a `published` receipt, the live index/worktree intentionally remain at the parent. A First Officer that can prove the same exclusive lease returned may invoke `fo-reconcile-completion.sh`. The primitive re-verifies the receipt objects, attached ref, parent tree, entity/artifact OIDs, clean index/worktree, absence of untracked files and index lock, and exact lease identity; then restores only the literal entity path and checks a clean postcondition. Dirty, concurrent, OID/ref/lease mismatch, or uncertain ownership fails closed before reconcile. Only a successful checkpoint may be followed by a separate Contract 1 entry in the same run.

**Contract 1 variant — First Officer feedback-stage routing**: the First Officer also owns routing a rejected stage back to its declared `feedback-to` target. Its commit subject, and only its subject, MUST match `feedback: <nonempty summary> <rejected-stage> cycle <N> to <target-stage>`, where `N` is a bounded canonical decimal (`[1-9][0-9]{0,8}`, at most 9 digits — safely inside both IEEE-754 double integer precision and the POSIX-guaranteed minimum 32-bit arithmetic range used to compare it). The graph gate above still runs first — a `feedback:` subject can never bless a transition whose edge is not *some* declared next-stage or feedback-to edge. That shared gate alone is not sufficient: because it accepts EITHER a next-stage edge OR a feedback-to edge, the feedback-stage receipt path separately re-resolves `<rejected-stage>`'s OWN declared `feedback-to` target at the parent revision and requires `<target-stage>` to equal it exactly — a `feedback:` subject riding a merely-legal forward (next-stage) edge is rejected even with a matching body record (feedback-to-edge bind). Once the edge is legal and specifically bound, every mutated entity's parent status MUST equal `<rejected-stage>` and resulting status MUST equal `<target-stage>`, and the entity's `### Feedback Cycles` body MUST carry exactly one record for that entity matching cycle, `rejected_stage`, `feedback_to`, `captain_decision: fix`, a valid UTC RFC3339 `routed_at`, and a `verify_artifact` of the form `<rejected-stage>.md@<7-40 lowercase hex>` — cycle is compared as a forced string, never a coerced number, closing an IEEE-754 numeric-string collision on unbounded cycle values. That matching record MUST additionally be freshly introduced BY this commit: it is compared against the same entity's PARENT revision, and a record already present, byte-identical, in the parent cannot satisfy the bind (stale cycle-record replay is rejected). Its cycle number MUST also be strictly greater than every other well-formed cycle already on record for that entity at the parent revision (monotonic cycle numbering). The record-matching parser tracks Markdown fence state (` ``` `/`~~~`, 0-3 leading spaces, CommonMark open/close semantics) and ignores every line inside an open fence; indented (4-space/tab) example blocks are excluded incidentally because every heading/field regex is column-anchored. A `### Feedback Cycles` heading or `- cycle:` record written inside a fenced or indented documentation example therefore can never authorize a transition (fence-safety). The body record is a consistency bind, not the authority — the subject line alone authorizes the transition, and prose or a mismatched/missing/duplicated/stale/non-monotonic/fenced record cannot substitute for it. Dropping any one of these binds turns a forged `feedback: … to <anystage>` subject into an authority leak.

**Mutation and shared graph gate**: mutation means a changed `status:` value inside YAML frontmatter between a commit's parent and the commit. Body examples are not mutations, and a genuinely new entity with no parent status is exempt. For existing entities, C14 validates every before/after pair against the parent-revision workflow graph before considering either contract. A legal sibling path or receipt text cannot hide an undeclared transition.

**Frontmatter authority and legacy boundary**: canonical folder entities use the strict `status:` plus `stage_outputs:` authority tail; `stage_outputs` is the sole machine owner of produced-stage links. `render-stage-links.sh --entity=<path>` is read-only and prints the complete derived Markdown table to stdout. Historical `stage-artifact-links` body sections are opaque, non-authoritative bytes and never authorize transition or completion. Legacy flat or body-table-only entities are Contract-1-only: stage workers append their report and return to the First Officer without migration, status mutation, or Contract 2.

**Failure modes**:

1. An arbitrary manual status edit carries no exact FO-entry subject and is rejected; historical body tables never authorize status mutation, and completion never authorizes it.
2. A stage worker advances to the next stage instead of returning to the First Officer, collapsing two independently auditable contracts into one ambiguous operation.
3. Invoking the helper on a legacy, dirty, or stale entity is rejected; bypassing its ref CAS or FO lease checkpoint can create frontmatter/body drift across sessions.
4. A receipt is evaluated before workflow legality, allowing skipped, backward, or mixed-path mutations to pass.
5. Squash merging can collapse branch-local receipts; C14 therefore catches bypasses on the pre-merge `merge-base..HEAD` range rather than claiming post-squash reconstruction.

**Executable check** (Tier A automated): `check_entity_status_via_advance_stage_only()` scans current-branch commits for folder and flat entity mutations. It parses the parent workflow graph, validates every transition first, then accepts only an exact subject-line Contract 1 receipt — a stage-entry (`dispatch:`/`advance:`) receipt via `_commit_has_fo_stage_entry_receipt()` or a feedback-stage (`feedback:`) receipt via `_commit_has_fo_feedback_stage_receipt()`. The latter re-resolves the specific feedback-to edge via `_workflow_feedback_to_target_at_rev()` and binds cycle, captain decision, and verify evidence through `_entity_has_fo_feedback_cycle_record()`, which itself binds freshness and monotonic cycle ordering against the parent revision via `_feedback_cycle_well_formed_records_at_rev()` and ignores fenced/example content in both directions via the shared `_FEEDBACK_CYCLES_FENCE_AWK` fragment; Contract 2 and historical body tables never authorize status mutation. `check_frontmatter_stage_output_authority()` pins the schema owner, runs the renderer on a canonical temporary entity, proves byte/hash preservation and exact stdout/stderr, and detects external `spacedock` invocation with an isolated PATH probe. Repositories without `origin/main` are skipped gracefully for fixture compatibility.

**Explicit deferrals**: migration, rename, ID correlation, and status-presence behavior (Cases 32–35, 37–43, and 45) remain issue #36; merge-resolution and inherited-parent behavior (Cases 36 and 44) remain issue #37; authenticated, tool-owned receipt provenance remains issue #38. None is implied by these two structural contracts.

**Activation**: the contracts take effect from the checked-in C14 and stage-skill/helper wiring. A subsequent `/ship` uses them immediately; a currently active entity can use them as soon as its layout satisfies the applicable contract. No release-time data migration or backfill is required.

**Source**: source pitch `enforce-advance-stage-primitive-only` (sharp 2026-05-15); source evidence pitch-106 commit `898d006c`; C14 FO dispatch contract issue #30, narrowed to Cases 14–31 with deferrals #36–#38.
