# Ship-Flow — Auditable Autonomous Workflow for Claude 4.7 (v0.7.0-rc.4)
...
EM classifies each finding as `accepted`, `false_positive`, or `out_of_scope`;
accepted findings route back for fixes, while `false_positive`/`out_of_scope`
findings require evidence-bearing `gh api` replies in the exact review
comment/thread plus resolve/dismiss when permissions allow.
