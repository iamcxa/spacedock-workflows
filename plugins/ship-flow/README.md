# Ship-Flow — Auditable Autonomous Feature Delivery for AI Coding Agents (v0.8.2)
...
SO/EM also owns AI/external PR review adjudication for auto-merge readiness.
When reviewer findings conflict or appear false positive, FO routes them to
`ship-flow:science-officer-em` instead of attempting PR-author self-approval.
EM replies in the AI reviewer inline comment thread with `fixed`,
`push-back: false positive`, or `needs captain decision`, includes code/test
evidence plus SO/EM judgment rationale, then FO re-triggers the AI reviewer
gate. The gate re-reads the thread and either resolves accepted feedback or
pushes back in-thread; EM does not need to know whether the backend is a
short-term GitHub Actions rerun or a long-term GitHub App check rerun.
...
- `bin/review-thread-gate.mjs` — separate mechanical gate for GitHub review threads. Fails when an unresolved, non-outdated thread remains on the PR head and points FO at SO/EM adjudication: EM replies in-thread with `fixed`, `push-back: false positive`, or `needs captain decision`, includes evidence via `gh api`, then FO re-triggers the AI reviewer gate.
- `bin/auto-merge-readiness.mjs` (extended) — report-only readiness reporter. Consumes PR/check/review snapshots plus semantic-review and review-thread gate outputs; returns `ready`, `blocked`, or `unknown` with `nextAction`. Does not call `gh pr merge`, enable auto-merge, or mutate remote state. Pass `--required-independent-approvals <n>` to require approving reviewers who are not the PR author; PR-author self-approval is ignored and active `CHANGES_REQUESTED` reviews return `science_officer_em_adjudicate_review_feedback` so EM can answer each AI reviewer thread and re-trigger the AI gate.
...
- Adopter policy extends the plugin default. The built-in semantic packet
  ...
  `required_label` when they need Carlove-style `kc_pr_review` or domain-specific
  dimensions.
- `bin/review-thread-gate.mjs` is a separate mechanical gate for GitHub review threads. It fails when an unresolved, non-outdated thread remains on the PR head, which lets Copilot or another reviewer produce actionable comments without requiring human approval as the branch protection primitive. Blocking output now points FO at SO/EM adjudication: EM replies in-thread with `fixed`, `push-back: false positive`, or `needs captain decision`, includes evidence via `gh api`, then FO re-triggers the AI reviewer gate.
- `bin/auto-merge-readiness.mjs` is report-only. It consumes PR/check/review snapshots plus semantic-review and review-thread gate outputs, then returns `ready`, `blocked`, or `unknown` with `nextAction`. It does not call `gh pr merge`, enable auto-merge, or mutate remote state. `--required-independent-approvals <n>` requires approving reviewers who are not the PR author; self-review comments stay advisory, author self-approval is not a bypass, and active `CHANGES_REQUESTED` reviews block readiness with `science_officer_em_adjudicate_review_feedback` until EM answers threads and the AI gate passes.
