...
`superpowers:brainstorming` owns the HARD-GATE Q-loop for contradiction-resolution. **Do NOT re-teach this procedure.** Invoke it; Layer B below provides the Shape Up framing (5-category classifier, per-app design-system.md targeting, Material/Atlassian Principles tier separation, L2 strictness DCs).
...
1. Invoke `Skill: superpowers:brainstorming` for each contradiction pair.
...
Dispatch cross-review to `executer` teammate (or fresh sonnet per Rule A fallback).

5-factor rubric adapted for design stage:
| Factor | Assert |
|---|---|
...
Verdict: **PROCEED** / **VETO** (max 2 loops) / **PROMPT_CAPTAIN**. Reviewer-unresponsive fallback per Principle 6 Rule A: fresh sonnet default.
...
- `superpowers:brainstorming` — Layer A delegate for contradiction-resolution Q-loop
