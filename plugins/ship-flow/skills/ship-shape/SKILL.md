...
**Why mandatory**: when shape-worker composes shape.md straight from a directive, captain becomes a reviewer who rubber-stamps agent framing — loses sense of direction, accepts artifacts without owning the problem. Forcing questions reverse the flow: captain articulates verbally first, worker organizes captain's words second. Captain's brain models the answer before speaking; shape.md becomes audit trail of captain's own thinking, not agent inference. AI-augmentation cargo-cult prevention — do NOT substitute agent thinking for captain thinking. This is codified as INVARIANTS **Principle 17**: the human review surface is the shape/spec (never the agent-facing plan.md/execute.md); the FO confirms spec content with the captain in plain language rather than offering a plan review.
...
Hard stop before compose/acceptance: missing receipt block, duplicate receipt blocks, wrong root `stage`/`mode`/`compose_guard`, missing or invalid root `appetite`, invalid status, missing required delegate row, wrong required delegate phase, primary delegate `required: false`, missing invoked evidence, missing unavailable fallback/rationale, missing skipped rationale, or medium/big `pol-probe-advisor` skip. This receipt guard is separate from the 088/097 static Layer A declaration/parity checks; those checks prove skill docs declare delegation, while this artifact proves the shape proposal carried invocation/fallback/skip evidence.
...
The FO ALWAYS advances the entity to the `design` stage (skill: `ship-flow:ship-design`) — per INVARIANTS Principle 11, design always runs and the former `skip-when` clause is removed; C14's transition graph has no `shape→plan` edge. This trigger selects the **design lane**, not skip-vs-run: when ANY condition fires, design does full lane work (UI / domain / contract); when NONE fires, the entity walks ship-design's Phase 0 **trivial-pass** fast-path (minimal `design.md` + unconditional PROCEED) and then advances to `plan`. Run `## Phase 8.5 — Domain Registry Validation` before evaluating the trigger so non-UI domains are not misclassified because `domain:` was never recorded.
...
**Design trivial-pass hand-off** (G14 amended by contract-design gate; INVARIANTS Principle 11 — design always runs): shape ALWAYS emits `### Hand-off to Design` and advances to the design stage. When `affects_ui: false` AND `domain:` unset AND `design_required: false` AND `contract_decision_required: false`, design runs its Phase 0 trivial-pass fast-path — it is NOT skipped at the pipeline level (no `shape→plan` transition; C14 rejects it). If any signal is set, design does full lane work.

When `affects_ui: false` AND `domain:` is unset AND `design_required: false` AND `contract_decision_required: false` → shape's `### Hand-off to Design` omits `ui_surfaces` and `framework_detected`; sets `open_design_questions: []`; sets `open_contract_decisions: []`. The `### Hand-off to Plan` with `design-skipped: true` (single field) is then emitted by **ship-design's Phase 0 trivial-pass** (INVARIANTS Principle 11.3), NOT by shape — plan Step 1.6 reads it from `design.md`. Rationale: the `design-skipped: true` marker lets plan distinguish "design intentionally bypassed (trivial-pass)" from "block missing — error", while the entity still passes through the uniform `shape → design → plan` chain. Validated by `check-invariants.sh --check plan-imported-design-dcs-emitted`.
