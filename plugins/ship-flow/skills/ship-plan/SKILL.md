...
- Block has `design-skipped: true` → design intentionally bypassed only when the entity is non-UI (`affects_ui: false`), `domain:` unset, `design_required: false`, `contract_decision_required: false`, and no `open_contract_decisions` remain, OR the handoff also carries `captain-approved-design-bypass: true` with a rationale. For `affects_ui: true` without that explicit bypass, **BLOCKER** (status: blocked, reason: `ui design handoff skipped`) and bounce to design/FO. For `domain:` set, `design_required: true`, `contract_decision_required: true`, or non-empty `open_contract_decisions` with a design-skipped stub, **BLOCKER** (status: blocked, reason: `design-bearing decision skipped`) and bounce to design/FO; planner must not choose the domain contract, selector grammar, API vocabulary, tool protocol, DSL, schema, or message format. Otherwise log `## Plan Imported Design DCs: design-skipped (no UI surface, no matched domain, no design_required signal, no contract decision required, or captain-approved bypass)` and proceed.
...
| `ui_quality_contract` | import each group from `plugins/ship-flow/references/ui-quality-contract.md` into DCs, reviewer questions, or explicit N/A rows. Required groups: `copy`, `visual_hierarchy`, `color`, `typography`, `spacing`, `interaction_states`, and `source_safety`. | task wave touching the affected UI/component contract |
...
- **TDD ledger gate**: after drafting `plan.md` and before `### Hand-off to Execute`, run `python3 plugins/ship-flow/lib/validate-tdd-ledger.py --plan <entity-folder>/plan.md > <entity-folder>/tdd-ledger.txt 2>&1` for the human-readable pass/fail summary, then run `python3 plugins/ship-flow/lib/validate-tdd-ledger.py --plan <entity-folder>/plan.md --emit-jsonl > <entity-folder>/tdd-ledger.jsonl`. Finally run `python3 plugins/ship-flow/lib/validate-tdd-ledger.py --plan <entity-folder>/plan.md --require-ledger-jsonl <entity-folder>/tdd-ledger.jsonl` to prove the persisted ledger exists and matches the current plan. Failure blocks plan advance. This makes the `tdd_contract` authoritative at plan time rather than post-hoc extraction. The validator rejects missing contracts, low-confidence `red_command` / `green_command`, invalid `layer`, stale ledger JSONL, and code-bearing tasks that declare `layer: meta` when the inferred layer is L1-L7.
...
- UI quality contract import: when `### Hand-off to Plan` contains `ui_quality_contract`, read `plugins/ship-flow/references/ui-quality-contract.md` and turn every group (`copy`, `visual_hierarchy`, `color`, `typography`, `spacing`, `interaction_states`, `source_safety`) into DCs, reviewer questions, or explicit N/A rows. Missing coverage for any group is a plan gap because verify needs concrete evidence prompts.
...
- `tdd-ledger`: path to `tdd-ledger.jsonl`, validator command, pass/fail summary from `validate-tdd-ledger.py --plan`, and persisted-ledger proof from `validate-tdd-ledger.py --plan <plan.md> --require-ledger-jsonl <tdd-ledger.jsonl>`; do not hand off a new non-trivial plan whose ledger validation fails.
