...
# Pre-Landing Review Checklist (Critical Pass)
...
- **Specialist categories (handled by parallel subagents, NOT this checklist):** Test Gaps, Dead Code, Magic Numbers, Conditional Side Effects, Performance & Bundle Impact, Crypto & Entropy. See `specialists/` for these.

All findings get action via Fix-First classification: obvious mechanical fixes are routed as AUTO-FIX (execute stage applies on next bounce); genuinely ambiguous issues are presented as ASK at the verify gate for captain decision.
...
**AUTO-FIX class (bounce to execute):**
- [file:line] Problem → fix recommendation

**ASK class (captain gate decision):**
...
#### Completeness Gaps
- Shortcut implementations where the complete version would cost <30 minutes agent time (e.g., partial enum handling, incomplete error paths, missing edge cases that are straightforward to add)
...
All findings are classified into AUTO-FIX vs ASK class.
- AUTO-FIX class → routed to execute stage via feedback-to (mechanical fixes).
- ASK class → presented at verify gate for captain decision (substantive).
Critical findings lean toward ASK (they're riskier).
Informational findings lean toward AUTO-FIX (they're more mechanical).
```
...
This heuristic is referenced by both the critical pass and the panel-merge logic. It determines whether a finding is routed as AUTO-FIX (bounce to execute) or ASK (captain decision at verify gate).

```
AUTO-FIX (mechanical, bounce to execute):  ASK (captain judgment at verify gate):
