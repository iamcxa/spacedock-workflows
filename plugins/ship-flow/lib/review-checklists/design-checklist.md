...
- Removed `~/.claude/skills/gstack/bin/gstack-diff-scope` bash invocation.
  Trigger is now described in prose; ship-verify owns scope detection via
  its own portable `lib/review-scope.sh`.
-->
...
**Trigger:** Only run this checklist if the diff touches frontend files. ship-verify's scope detection sets `SCOPE_FRONTEND=true` when the diff includes:
- JSX / TSX / Vue / Svelte components
- CSS / SCSS / styled-components / Tailwind class changes
- Storybook stories or other UI fixtures
...
- **[LOW]** — Requires understanding visual intent. Present as: "Possible issue — verify visually or escalate to design-officer for visual review."
...
**LOW confidence items** → present as "Possible: [description]. Verify visually or escalate to design-officer." Never AUTO-FIX.
...
- [file:line] Possible issue — escalate to design-officer for visual check
```
