...
check "PR receipt example records self-approved PR_READY rule source" \
  "grep -q 'decision: self-approved' '${PR_MERGE_MOD}' && grep -q 'verdict: PR_READY' '${PR_MERGE_MOD}' && grep -Eq 'rule_source: docs/ship-flow/_mods/pr-merge.md v0\\.11\\.[0-9]+' '${PR_MERGE_MOD}'"
