...
echo ""
echo "Block 9: plan canonical_doc_actions must be consumed by review"
PLAN_ACTION_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-action-missing-review-outcome")"
run_checker "plan-action-missing-review-outcome" "${TMP_DIR}/plan-action.out" "${TMP_DIR}/plan-action.exit"
assert_exit "plan action missing review outcome exits 1" 1 "${TMP_DIR}/plan-action.exit"
assert_contains "plan action missing review outcome reports blocker" '^BLOCKER canonical_doc_actions: ARCHITECTURE\.md update from plan missing review outcome' "${TMP_DIR}/plan-action.out"
assert_read_only "plan action fixture remains unchanged" "plan-action-missing-review-outcome" "$PLAN_ACTION_BEFORE"

echo ""
echo "Block 10: plan canonical_doc_actions success path"
PLAN_ACTION_COMPLETE_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-actions-complete")"
run_checker "plan-actions-complete" "${TMP_DIR}/plan-actions-complete.out" "${TMP_DIR}/plan-actions-complete.exit"
assert_exit "plan actions complete exits 0" 0 "${TMP_DIR}/plan-actions-complete.exit"
assert_contains "plan actions complete consumes architecture update" '^PASS canonical_doc_actions: ARCHITECTURE\.md update from plan consumed' "${TMP_DIR}/plan-actions-complete.out"
assert_contains "plan actions complete consumes product skip" '^PASS canonical_doc_actions: PRODUCT\.md skip from design consumed' "${TMP_DIR}/plan-actions-complete.out"
assert_not_contains "plan actions complete emits no canonical action blockers" '^BLOCKER canonical_doc_actions:' "${TMP_DIR}/plan-actions-complete.out"
assert_read_only "plan actions complete fixture remains unchanged" "plan-actions-complete" "$PLAN_ACTION_COMPLETE_BEFORE"

echo ""
echo "Block 11: malformed plan canonical_doc_actions block"
MALFORMED_ACTION_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-action-malformed")"
run_checker "plan-action-malformed" "${TMP_DIR}/malformed-action.out" "${TMP_DIR}/malformed-action.exit"
assert_exit "malformed plan action exits 1" 1 "${TMP_DIR}/malformed-action.exit"
assert_contains "malformed plan action reports invalid action blocker" '^BLOCKER canonical_doc_actions: ARCHITECTURE\.md invalid action updates' "${TMP_DIR}/malformed-action.out"
assert_read_only "malformed plan action fixture remains unchanged" "plan-action-malformed" "$MALFORMED_ACTION_BEFORE"

echo ""
echo "Block 12: every canonical doc requires exactly one plan action row"
MISSING_REQUIRED_ROW_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-action-missing-required-row")"
run_checker "plan-action-missing-required-row" "${TMP_DIR}/missing-required-row.out" "${TMP_DIR}/missing-required-row.exit"
assert_exit "missing required plan row exits 1" 1 "${TMP_DIR}/missing-required-row.exit"
assert_contains "missing required plan row reports ROADMAP blocker" '^BLOCKER canonical_doc_actions: ROADMAP\.md missing plan action row' "${TMP_DIR}/missing-required-row.out"
assert_read_only "missing required plan row fixture remains unchanged" "plan-action-missing-required-row" "$MISSING_REQUIRED_ROW_BEFORE"

echo ""
echo "Block 13: invalid canonical doc names block"
INVALID_DOC_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-action-invalid-doc")"
run_checker "plan-action-invalid-doc" "${TMP_DIR}/invalid-doc.out" "${TMP_DIR}/invalid-doc.exit"
assert_exit "invalid doc exits 1" 1 "${TMP_DIR}/invalid-doc.exit"
assert_contains "invalid doc reports blocker" '^BLOCKER canonical_doc_actions: invalid doc ARCHITECTURE' "${TMP_DIR}/invalid-doc.out"
assert_read_only "invalid doc fixture remains unchanged" "plan-action-invalid-doc" "$INVALID_DOC_BEFORE"

echo ""
echo "Block 14: skip actions must be carried into review consumption"
SKIP_NOT_CONSUMED_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-action-skip-not-consumed")"
run_checker "plan-action-skip-not-consumed" "${TMP_DIR}/skip-not-consumed.out" "${TMP_DIR}/skip-not-consumed.exit"
assert_exit "skip not consumed exits 1" 1 "${TMP_DIR}/skip-not-consumed.exit"
assert_contains "skip not consumed reports blocker" '^BLOCKER canonical_doc_actions: PRODUCT\.md skip from plan not carried into review outcome' "${TMP_DIR}/skip-not-consumed.out"
assert_read_only "skip not consumed fixture remains unchanged" "plan-action-skip-not-consumed" "$SKIP_NOT_CONSUMED_BEFORE"

echo ""
echo "Block 15: consumed plan action must match plan action"
CONSUMED_ACTION_MISMATCH_BEFORE="$(hash_dir "${FIXTURE_ROOT}/plan-action-consumed-action-mismatch")"
run_checker "plan-action-consumed-action-mismatch" "${TMP_DIR}/consumed-action-mismatch.out" "${TMP_DIR}/consumed-action-mismatch.exit"
assert_exit "consumed action mismatch exits 1" 1 "${TMP_DIR}/consumed-action-mismatch.exit"
assert_contains "consumed action mismatch reports blocker" '^BLOCKER canonical_doc_actions: ARCHITECTURE\.md review consumption plan action skip does not match update' "${TMP_DIR}/consumed-action-mismatch.out"
