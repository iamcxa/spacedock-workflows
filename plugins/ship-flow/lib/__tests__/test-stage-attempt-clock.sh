...
if [ "$CLOCK_CASE" = full ] || [ "$CLOCK_CASE" = nonterminal ] || [ "$CLOCK_CASE" = return-budget ] ||
  ...
  if [ "$CLOCK_CASE" = full ] || [ "$CLOCK_CASE" = nonterminal ]; then
  ...
  return_budget_case() {
    ...
    if [ "$CLOCK_CASE" = full ] || [ "$CLOCK_CASE" = return-authority ]; then
      cp "$open_wal" "$wal"
      rm -f "$returned"
    ...
    if [ "$CLOCK_CASE" = full ] || [ "$CLOCK_CASE" = return-outcome-authority ]; then
  ...
  if [ "$CLOCK_CASE" = full ] || [ "$CLOCK_CASE" = elapsed-sync ]; then
    elapsed_sync_case
  fi
